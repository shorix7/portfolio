...
Public Class Form9
    Sub Dobavlenie_zap()
        ...
        Try
            Dim Command As New OleDbCommand("Insert Into [Приказы по студентам] ([Регистрационный номер], [Дата регистрации], [Краткое содержание], [Количество листов], [Исполнитель], [Куда передан приказ], [Отметка о помещении в дело]) values ('" & p1 & "', '" & p2 & "', '" & p3 & "', '" & p4 & "', '" & p5 & "', '" & p6 & "', '" & p7 & "')", Connector)
            Connector.Open()
    ...
    Sub Editor_zap()
        Try
            Dim Command As New OleDbCommand("UPDATE [Приказы по студентам] SET [Регистрационный номер]='" & p1 & "', [Дата регистрации]='" & p2 & "', [Краткое содержание]='" & p3 & "', [Количество листов]='" & p4 & "', [Исполнитель]='" & p5 & "', [Куда передан приказ]='" & p6 & "', [Отметка о помещении в дело]='" & p7 & "' WHERE ([Регистрационный номер] Like '" & ListView1.SelectedItems.Item(0).Text & "')", Connector)
            Connector.Open()
            Command.ExecuteNonQuery()
            Connector.Close()
    ...
    Private Sub Form9_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        Try
            ...
            If ToolStripTextBox1.Text = "" Then
                Poisk = "SELECT * FROM [Приказы по студентам] ORDER BY [Приказы по студентам].[Код] DESC"
            Else
                Poisk = "SELECT * FROM [Приказы по студентам] where (([Приказы по студентам].[" & ToolStripComboBox1.Text & "]) Like '%" & ToolStripTextBox1.Text & "%')"
            End If
            ...
            While DataReader.Read() = True
                ...
                CountForm5 = ListView1.Items.Count
            End While
            DataReader.Close()
            Connector.Close()
    ...
    Private Sub ListView1_MouseDoubleClick(sender As Object, e As MouseEventArgs) Handles ListView1.MouseDoubleClick
        Try
            ...
            Form10.Close()
            Form10.Text = "Редактирование записи"
            Form10.Show()
        Catch ex As Exception
            MessageBox.Show(ex.Message)
        End Try
